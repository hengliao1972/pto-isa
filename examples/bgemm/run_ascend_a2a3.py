...
def _is_arm64_host() -> bool:
    """Best-effort host arch check (used to gate Ascend board-only paths)."""
    import platform

    return platform.machine() in ("aarch64", "arm64")


def _bgemm_step_ptoas_text(
    *,
    num_tiles: int = 8,
    tile_m: int = 64,
    tile_n: int = 128,
    ...
    """
    Emit a small PTO-AS program (ptoas dialect) for the bgemm "step" accuracy test.

    Kernel ABI: mpmd (args: __gm__ int64_t*)
      args[0] = task_id (tile_id)
      args[1] = A base pointer (f32)
      args[2] = B base pointer (f32)
      args[3] = C base pointer (f32)
    """
    a_rows = int(num_tiles) * int(k_tiles) * int(tile_m)
    a_cols = int(tile_k)
    b_rows = int(k_tiles) * int(num_tiles) * int(tile_k)
    b_cols = int(tile_n)
    c_rows = int(num_tiles) * int(tile_m)
    c_cols = int(tile_n)

    # Row-major (ND) strides for 2D tensors.
    a_s0, a_s1 = a_cols, 1
    b_s0, b_s1 = b_cols, 1
    c_s0, c_s1 = c_cols, 1

    def _tile(loc: str, element: str, rows: int, cols: int, *, blayout=None, slayout=None) -> str:
        parts = [f"loc={loc}", f"element={element}", f"rows={rows}", f"cols={cols}"]
        if blayout is not None:
            parts.append(f"blayout={blayout}")
        if slayout is not None:
            parts.append(f"slayout={slayout}")
        return "!pto.tile<" + ", ".join(parts) + ">"

    # Constants for index math.
    a_tile_row_stride = int(tile_m)  # rows per A tile
    b_tile_row_stride = int(num_tiles) * int(tile_k)  # rows between successive k-tiles for fixed tile_id
    a_base_mul = int(k_tiles) * int(tile_m)  # rows per tile_id in A

    lines: list[str] = []
    lines.append("// bgemm_step.pto (auto-generated by examples/bgemm/run_ascend_a2a3.py)")
    lines.append("// PTO-AS for ptoas (mpmd ABI).")
    lines.append("prologue")
    lines.append(
        f"%A = pto.make_tensor_view %arg1, dtype=f32, shape=[{a_rows},{a_cols}], stride=[{a_s0},{a_s1}], layout=ND"
    )
    lines.append(
        f"%B = pto.make_tensor_view %arg2, dtype=f32, shape=[{b_rows},{b_cols}], stride=[{b_s0},{b_s1}], layout=ND"
    )
    lines.append(
        f"%C = pto.make_tensor_view %arg3, dtype=f32, shape=[{c_rows},{c_cols}], stride=[{c_s0},{c_s1}], layout=ND"
    )
    # Tile layout conventions follow the ptoas GEMM examples/exporter:
    # - Mat/Left/Acc: blayout=ColMajor, slayout=RowMajor
    # - Right:        blayout=RowMajor, slayout=ColMajor
    lines.append(f"%a_mat = pto.alloc_tile : {_tile('Mat', 'f32', tile_m, tile_k, blayout='ColMajor', slayout='RowMajor')}")
    lines.append(f"%a_left = pto.alloc_tile : {_tile('Left', 'f32', tile_m, tile_k, blayout='ColMajor', slayout='RowMajor')}")
    lines.append(f"%b_mat = pto.alloc_tile : {_tile('Mat', 'f32', tile_k, tile_n, blayout='ColMajor', slayout='RowMajor')}")
    lines.append(f"%b_right = pto.alloc_tile : {_tile('Right', 'f32', tile_k, tile_n, blayout='RowMajor', slayout='ColMajor')}")
    lines.append(f"%c_acc = pto.alloc_tile : {_tile('Acc', 'f32', tile_m, tile_n, blayout='ColMajor', slayout='RowMajor')}")
    lines.append("")

    # task_id -> row offsets
    lines.append("get_task_id %bid")
    lines.append(f"imul %a_base, %bid, {a_base_mul}")
    lines.append(f"imul %b_base, %bid, {tile_k}")
    lines.append(f"imul %c_base, %bid, {tile_m}")
    lines.append("")

    # K reduction: unrolled (k_tiles is small / fixed for this test).
    for k in range(int(k_tiles)):
        a_off = int(k) * int(a_tile_row_stride)
        b_off = int(k) * int(b_tile_row_stride)
        a_row = f"%a_row_{k}"
        b_row = f"%b_row_{k}"
        if k == 0:
            lines.append(f"iadd {a_row}, %a_base, {a_off}")
            lines.append(f"iadd {b_row}, %b_base, {b_off}")
            lines.append(f"tload %a_mat, %A[{a_row}, 0]")
            lines.append(f"tload %b_mat, %B[{b_row}, 0]")
            lines.append("tmov %a_left, %a_mat")
            lines.append("tmov %b_right, %b_mat")
            lines.append("tmatmul %c_acc, %a_left, %b_right")
        else:
            lines.append(f"iadd {a_row}, %a_base, {a_off}")
            lines.append(f"iadd {b_row}, %b_base, {b_off}")
            lines.append(f"tload %a_mat, %A[{a_row}, 0]")
            ...
            lines.append("tmov %a_left, %a_mat")
            lines.append("tmov %b_right, %b_mat")
            lines.append("tmatmul_acc %c_acc, %c_acc, %a_left, %b_right")
        lines.append("")

    # Store C tile: shape [tile_m, tile_n] at row offset c_base.
    lines.append("tstore %C[%c_base, 0], %c_acc")
...
def generate_code():
    ...
    # For real Ascend A2/A3 hardware runs, prefer generating a ptoas-compatible PTO-AS
    # program directly (avoids dependency on the legacy `pto_as` Python DSL).
    if CONFIG["target_platform"] == "ascend_a2a3":
        platform = CONFIG["target_platform"]
        platform_dir = ensure_dir(os.path.join(OUTPUT_DIR, platform))
        out_pto = os.path.join(platform_dir, "bgemm_step.pto")
        pto_text = _bgemm_step_ptoas_text()
        with open(out_pto, "w", encoding="utf-8") as f:
            f.write(pto_text)
        print(f"  Generated PTO-AS (ptoas): {out_pto}")
        return True
...
def compile_code():
    ...
    # For real Ascend A2/A3 hardware runs, compile the PTO-AS to CCE C++ via `ptoas`.
    if platform == "ascend_a2a3":
        print("  Compiling PTO-AS via ptoas (hardware path)...")
        pto_path = os.path.join(platform_dir, "bgemm_step.pto")
        out_cpp = os.path.join(platform_dir, "bgemm_step.cpp")
        if not os.path.exists(pto_path):
            print(f"  Error: missing PTO-AS input: {pto_path}")
            return False
        if not _is_arm64_host():
            print("  Note: ptoas NPU compilation requires running on an aarch64/arm64 host.")
            print("        (Current host arch does not match; skipping ptoas compilation.)")
            return False
        try:
            ...
            if not ascend_home:
                ascend_home = os.fspath(Path.home() / "Ascend" / "ascend-toolkit" / "latest")
            ...
            cfg = PtoasConfig(
                ...
                log_path=Path(platform_dir) / "ptoas.log",
                print_cmd=True,
            )
            compile_pto_to_cce_cpp(pto_path=Path(pto_path), out_cpp=Path(out_cpp), cfg=cfg)
            print(f"  ✓ Generated CCE C++: {out_cpp}")
            print(f"  ✓ ptoas log:         {os.path.join(platform_dir, 'ptoas.log')}")
            return True
        except Exception as e:
            print(f"  ✗ ptoas compilation failed: {e}")
            return False
...
def run_task_dump():
    ...
    # Hardware path: run the step kernel on-device via ref_runtime + ptoas.
    if platform == "ascend_a2a3":
        if not _is_arm64_host():
            print("  Note: Ascend A2/A3 device run requires aarch64/arm64 host.")
            return False

        try:
            import numpy as np
        except ImportError:
            print("  Error: NumPy is required for the Ascend device run path.")
            return False
        ...
        try:
            ...
            print("  Hint: ensure you are running on the board and that `ASCEND_HOME_PATH` is set.")
            return False

        # Ensure ASCEND_HOME_PATH is set (BinaryCompiler and ptoas both rely on it).
        from pathlib import Path
        ...
        aic_blocks = int(os.environ.get("PTO_AIC_BLOCKS", "24"))

        # Kernel source compiled by ptoas (compile_code step).
        kernel_cpp = os.path.join(platform_dir, "bgemm_step.cpp")
        if not os.path.exists(kernel_cpp):
            print(f"  Error: missing CCE C++ kernel: {kernel_cpp}")
            print("  Run the Compilation step first (or rerun the whole script).")
            return False

        # Test configuration (must match run_accuracy_test defaults).
        num_tiles = 8
        tile_m = 64
        tile_n = 128
        tile_k = 64
        ...
        test_input_floats = 1024 * 1024
        test_output_floats = 1024 * 1024
        test_input_bytes = test_input_floats * 4
        test_output_bytes = test_output_floats * 4
        half_bytes = test_input_bytes // 2

        print(f"  Device: {device_id}  aic_blocks: {aic_blocks}")
        print(f"  Kernel: {kernel_cpp}")
        print(f"  Params: num_tiles={num_tiles}, tile_m={tile_m}, tile_n={tile_n}, tile_k={tile_k}, k_tiles={k_tiles}")

        runner = pto_runtime.DeviceRunner.get()
        rc = int(runner.init(int(device_id), int(aic_blocks)))
        if rc != 0:
            print(f"  Error: DeviceRunner.init failed: rc={rc}")
            return False

        try:
            # Compile and load the kernel as func_id=0 (AIC/cube kernel_type=0 for CompileAndLoadKernel).
            rc = int(runner.compile_and_load_kernel(0, kernel_cpp, os.fspath(Path(ROOT_DIR)), core_type=0))
            if rc != 0:
                print(f"  Error: compile_and_load_kernel failed: rc={rc}")
                return False

            # Allocate buffers.
            host_input = (np.arange(test_input_floats, dtype=np.float32) % 1000).astype(np.float32) / np.float32(1000.0)
            host_output = np.zeros((test_output_floats,), dtype=np.float32)

            dev_in = int(runner.allocate_tensor(int(test_input_bytes)))
            dev_out = int(runner.allocate_tensor(int(test_output_bytes)))
            if not dev_in or not dev_out:
                print("  Error: allocate_tensor failed")
                return False

            try:
                rc = int(runner.copy_to_device(dev_in, host_input))
                if rc != 0:
                    print(f"  Error: copy_to_device(input) failed: rc={rc}")
                    return False
                rc = int(runner.copy_to_device(dev_out, host_output))
                if rc != 0:
                    print(f"  Error: copy_to_device(output) failed: rc={rc}")
                    return False

                # Build graph: one task per output tile.
                graph = pto_runtime.Graph()
                b_ptr = int(dev_in) + int(half_bytes)
                for tile_id in range(int(num_tiles)):
                    graph.add_task([int(tile_id), int(dev_in), int(b_ptr), int(dev_out)], func_id=0, core_type=1)

                # Run (warmup + timed).
                warmup = int(os.environ.get("PTO_WARMUP", "1"))
                iters = int(os.environ.get("PTO_ITERS", "1"))

                for _ in range(max(0, warmup)):
                    rc = int(runner.run(graph, 1))
                    if rc != 0:
                        print(f"  Error: warmup run failed: rc={rc}")
                        return False
                ...
                for _ in range(max(1, iters)):
                    rc = int(runner.run(graph, 1))
                    if rc != 0:
                        print(f"  Error: run failed: rc={rc}")
                        return False
                t1 = time.perf_counter()

                avg_ms = (t1 - t0) * 1e3 / float(max(1, iters))
                print(f"  Runtime: avg_time_ms={avg_ms:.4f} (iters={iters}, tasks={num_tiles})")

                rc = int(runner.copy_from_device(host_output, int(dev_out)))
                if rc != 0:
                    print(f"  Error: copy_from_device(output) failed: rc={rc}")
                    return False

                # Save data for run_accuracy_test().
                ...
                host_input.tofile(input_file)
                host_output.tofile(output_file)
                with open(params_file, "w", encoding="utf-8") as f:
                    f.write(f"input_size={test_input_bytes}\n")
                    f.write(f"output_size={test_output_bytes}\n")
                    f.write(f"num_tiles={num_tiles}\n")
                    f.write(f"tile_m={tile_m}\n")
                    f.write(f"tile_n={tile_n}\n")
                    f.write(f"tile_k={tile_k}\n")
                    f.write(f"k_tiles={k_tiles}\n")
                print(f"  Saved: {input_file}")
                print(f"  Saved: {output_file}")
                print(f"  Saved: {params_file}")
            finally:
                try:
                    runner.free_tensor(int(dev_in))
                except Exception:
                    pass
                try:
                    runner.free_tensor(int(dev_out))
                except Exception:
                    pass
        finally:
            try:
