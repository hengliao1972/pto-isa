...
/**
 * InCore Function: tile_muls_64
 * Single-core tile computation function.
 * Called by PTO Runtime as a scheduled task.
 * NOT a kernel entry - use tile_muls_64_kernel_wrapper() to launch as kernel.
 */
class tile_muls_64InCore {
public:
    // Single-core constructor - no block coordination
    __aicore__ inline tile_muls_64InCore() {}

    // Initialize with global memory pointers
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR output) {
        inputGm.SetGlobalBuffer((__gm__ float*)input);
        outputGm.SetGlobalBuffer((__gm__ float*)output);
        pipe.InitBuffer(inQueueX, 1, 65536);
        pipe.InitBuffer(outQueueY, 1, 65536);
    }
...
private:
    ...
    __aicore__ inline void CopyOut() {
        ...
        DataCopy(outputGm, yLocal, 16384);
 ...
 */
__aicore__ inline void tile_muls_64(
 ...
 */
extern "C" __global__ __aicore__ void tile_muls_64_kernel(GM_ADDR input, GM_ADDR output) {
    tile_muls_64InCore op;
