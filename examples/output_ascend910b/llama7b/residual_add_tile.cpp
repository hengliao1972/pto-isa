...
/**
 * InCore Function: residual_add_tile
 * Single-core tile computation function.
 * Called by PTO Runtime as a scheduled task.
 * NOT a kernel entry - use residual_add_tile_kernel_wrapper() to launch as kernel.
 */
class residual_add_tileInCore {
public:
    // Single-core constructor - no block coordination
    __aicore__ inline residual_add_tileInCore() {}

    // Initialize with global memory pointers
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR output) {
        inputGm.SetGlobalBuffer((__gm__ float*)input);
        outputGm.SetGlobalBuffer((__gm__ float*)output);
        pipe.InitBuffer(inQueueX, 1, 49152);
        pipe.InitBuffer(outQueueY, 1, 49152);
    }
...
private:
    ...
    __aicore__ inline void CopyOut() {
        ...
        DataCopy(outputGm, yLocal, 12288);
 ...
 */
__aicore__ inline void residual_add_tile(
 ...
 */
extern "C" __global__ __aicore__ void residual_add_tile_kernel(GM_ADDR input, GM_ADDR output) {
    residual_add_tileInCore op;
