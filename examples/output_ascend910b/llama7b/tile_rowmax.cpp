 ...
 * NOT a kernel entry - use tile_rowmax_kernel_wrapper() to launch as kernel.
 */
class tile_rowmaxInCore {
public:
    // Single-core constructor - no block coordination
    __aicore__ inline tile_rowmaxInCore() {}

    // Initialize with global memory pointers
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR output) {
        inputGm.SetGlobalBuffer((__gm__ float*)input);
        outputGm.SetGlobalBuffer((__gm__ float*)output);
        pipe.InitBuffer(inQueueX, 1, 16512);
        pipe.InitBuffer(outQueueY, 1, 16512);
    }
 ...
 */
__aicore__ inline void tile_rowmax(
 ...
 */
extern "C" __global__ __aicore__ void tile_rowmax_kernel(GM_ADDR input, GM_ADDR output) {
    tile_rowmaxInCore op;
