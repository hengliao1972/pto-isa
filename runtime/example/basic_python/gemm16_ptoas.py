...
def _find_runtime_build_root(start: Path) -> Path:
    ...
    raise RuntimeError(
        ...
        "Build runtime first: `cd runtime && mkdir -p build && cd build && cmake .. && make -j`."
    )
...
def main() -> int:
    try:
        ...
        print("Build the Python bindings first:")
        print("  cd runtime && mkdir -p build && cd build && cmake .. -DBUILD_PYTHON_BINDINGS=ON && make -j")
        print("Then set PYTHONPATH (from runtime/build):")
        print("  export PYTHONPATH=$(pwd)/python:$PYTHONPATH")
        return 1
    ...
    # Locate runtime build outputs (aicpu/aicore) from current directory.
    build_root = _find_runtime_build_root(Path.cwd().resolve())
    ...
    rc = int(runner.init(device_id, 1, os.fspath(aicpu_so), os.fspath(aicore_kernel)))
    if rc != 0:
    ...
    cfg = PtoasConfig(
        ...
        ascend_home=Path(os.environ.get("ASCEND_HOME_PATH", "")) if os.environ.get("ASCEND_HOME_PATH") else Path.home() / "Ascend" / "ascend-toolkit" / "latest",
        repo_root=repo_root,
        timeout_s=timeout_s,
