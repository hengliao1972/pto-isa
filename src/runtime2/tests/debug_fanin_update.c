        ...
        if (sched->task_state[slot] == PTO2_TASK_PENDING) {
            ...
            while (cur > 0) {
                ...
                printf("  Producer %d: state=%d, fanout_head=%d, fanout_count=%d\n",
                       prod_id, sched->task_state[prod_slot], prod->fanout_head, prod->fanout_count);
                
                // 检查 producer 的 fanout list 是否包含 consumer i
                int found = 0;
                int fanout_len = 0;
                int32_t fo_cur = prod->fanout_head;
                while (fo_cur > 0) {
                    PTO2DepListEntry* fo_e = pto2_dep_pool_get(dep_pool, fo_cur);
                    if (!fo_e) { printf("    -> NULL entry at %d!\n", fo_cur); break; }
                    fanout_len++;
                    if (fo_e->task_id == i) found = 1;
                    fo_cur = fo_e->next_offset;
                }
                printf("    -> fanout list len=%d, contains consumer %d: %s\n",
                       fanout_len, i, found ? "YES" : "NO");
