...
int main() {
    // Use only 1 cube worker and 1 vector worker to reduce concurrency
    ...
    for (int i = 0; i < 60; i++) {
        ...
        if (sched->tasks_completed == 8192 && sched->tasks_consumed == 8192) {
            printf("SUCCESS after %.1fs: completed=%lld, consumed=%lld\n",
                   (i+1)*0.5, (long long)sched->tasks_completed, (long long)sched->tasks_consumed);
            break;
        }
    }
