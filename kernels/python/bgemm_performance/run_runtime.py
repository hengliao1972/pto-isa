...
def _find_runtime_build_root(start: Path) -> Path:
    for p in [start, *start.parents]:
        if (p / "aicpu" / "libaicpu_graph_kernel.so").exists() and (p / "aicore" / "kernel.o").exists():
            return p
    raise RuntimeError(
        "failed to locate runtime build root (expected aicpu/libaicpu_graph_kernel.so and aicore/kernel.o). "
        "Build runtime first: `cd runtime && mkdir -p build && cd build && cmake .. -DBUILD_PYTHON_BINDINGS=ON && make -j`."
    )
...
def main() -> int:
    ...
    try:
        ...
        print("error: cannot import pto_runtime; build it first:", file=sys.stderr)
        print("  cd runtime && mkdir -p build && cd build && cmake .. -DBUILD_PYTHON_BINDINGS=ON && make -j", file=sys.stderr)
        print("  export PYTHONPATH=$PWD/python:$PYTHONPATH", file=sys.stderr)
        return 2
    ...
    _kv("ascend_home", Path(args.ascend_home).resolve())
    ...
    _stage("Init runtime")
    build_root = _find_runtime_build_root((_REPO_ROOT / "runtime" / "build").resolve())
    aicpu_so = build_root / "aicpu" / "libaicpu_graph_kernel.so"
    aicore_kernel = build_root / "aicore" / "kernel.o"
    _kv("runtime_build", build_root)
    _kv("aicpu_so", aicpu_so)
    _kv("aicore_stub", aicore_kernel)

    runner = pto_runtime.DeviceRunner.get()
    rc = int(runner.init(int(args.device), int(args.aic_blocks), os.fspath(aicpu_so), os.fspath(aicore_kernel)))
    if rc != 0:
