...
### 2) Build the runtime Python bindings

```bash
cmake -S runtime -B runtime/build -DBUILD_PYTHON_BINDINGS=ON
cmake --build runtime/build -j
export PYTHONPATH=$PWD/runtime/build/python:$PYTHONPATH
```
...
- `error: cannot import pto_runtime`: verify `PYTHONPATH=$PWD/runtime/build/python:$PYTHONPATH`.
